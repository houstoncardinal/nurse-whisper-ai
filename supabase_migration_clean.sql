-- =====================================================
-- NurseScribe AI - Clean Supabase Migration
-- =====================================================
-- This migration creates all necessary tables from scratch
-- Handles existing tables and column conflicts properly
-- =====================================================

-- Drop existing tables if they exist (in reverse order to handle dependencies)
DROP TABLE IF EXISTS system_settings CASCADE;
DROP TABLE IF EXISTS time_saver_shortcuts CASCADE;
DROP TABLE IF EXISTS icd10_suggestions CASCADE;
...
DROP FUNCTION IF EXISTS log_audit_event(UUID, UUID, TEXT, TEXT, UUID, JSONB, INET, TEXT) CASCADE;
DROP FUNCTION IF EXISTS calculate_user_stats(UUID) CASCADE;
DROP FUNCTION IF EXISTS get_icd10_suggestions(TEXT, INTEGER) CASCADE;
...
-- =====================================================
CREATE TABLE icd10_suggestions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- =====================================================
CREATE TABLE time_saver_shortcuts (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID REFERENCES user_profiles(id) ON DELETE CASCADE,
    organization_id UUID REFERENCES organizations(id) ON DELETE CASCADE,
...
CREATE INDEX idx_note_metadata_user ON note_metadata(user_id);
CREATE INDEX idx_note_metadata_template ON note_metadata(template_id);
CREATE INDEX idx_note_metadata_status ON note_metadata(status);
CREATE INDEX idx_note_metadata_created ON note_metadata(created_at);
...
CREATE INDEX idx_export_history_note ON export_history(note_id);
...
CREATE INDEX idx_analytics_organization ON analytics(organization_id);
...
CREATE INDEX idx_icd10_keyword ON icd10_suggestions USING gin(keyword gin_trgm_ops);
CREATE INDEX idx_icd10_code ON icd10_suggestions(code);
CREATE INDEX idx_icd10_active ON icd10_suggestions(is_active);
